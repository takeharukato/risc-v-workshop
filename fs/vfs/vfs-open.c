 ...
 */
static int 
open_common(vfs_ioctx *ioctx, vnode *v, vfs_open_flags omode, int *fdp){
	int                     fd;
	 ...
	 * ファイルディスクリプタを獲得する
	 */
	rc = vfs_fd_alloc(ioctx, v, omode, &fd, &f);
	if ( rc != 0 ) {
 ...
 */
int
vfs_open(vfs_ioctx *ioctx, char *path, vfs_open_flags omode, int *fdp) {
	vnode *v;
	int   rc;
	int   fd;
