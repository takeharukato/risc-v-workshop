 ...
 */
static int
vm_unmap_common(vm_pgtbl pgt, vm_vaddr vaddr, vm_flags flags, vm_size size){
	int               rc;
	vm_paddr   map_paddr;
	vm_prot     map_prot;
	vm_flags   map_flags;
...
unmap_out:
	 ...
	 * マップ済みの領域を解放する
	 */
	for( rm_vaddr = sta_vaddr; cur_vaddr > rm_vaddr; ) {

		rc = hal_pgtbl_extract(dest, cur_vaddr, &rm_paddr, &rm_prot, &dest_flags,
		    &dest_pgsize);
		if ( rc != 0 )
			continue;  /* エラーケースを無視してアンマップを試みる */

		rm_prot = dest_prot;		/* 同一の保護属性を設定           */
		rm_flags = flags | dest_flags;  /* ページ割当て要否を引数から得る */

		/* ページをアンマップする */
		vm_unmap_common(dest, rm_vaddr, rm_flags, dest_pgsize);

		rm_vaddr += dest_pgsize;  /* 次のページ */
	}
...
vm_unmap(vm_pgtbl pgt, vm_vaddr vaddr, vm_flags flags, vm_size size){
	...
	for( rm_vaddr = sta_vaddr; end_vaddr > rm_vaddr; ) {
		...
		/* ページをアンマップする */
		vm_unmap_common(pgt, rm_vaddr, rm_flags, rm_pgsize);
